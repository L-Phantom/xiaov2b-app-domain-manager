  ...
  if [[ "$WEBMAN_PID" =~ ^[0-9]+$ ]]; then
    ...
    if [ -d "/proc/$WEBMAN_PID" ]; then
      ...
      if [ -n "$WEBMAN_CWD" ] && [ "$WEBMAN_CWD" != "$TARGET_REALPATH" ]; then
        echo "Skip Webman reload: cached PID $WEBMAN_PID belongs to $WEBMAN_CWD, not $TARGET_REALPATH"
        RESTART_WEBMAN=0
      elif [ -n "$WEBMAN_CMD" ] && [[ "$WEBMAN_CMD" != *"webman.php"* ]]; then
        echo "Skip Webman reload: cached PID $WEBMAN_PID is not a webman.php process"
        RESTART_WEBMAN=0
      fi
    elif ! kill -0 "$WEBMAN_PID" 2>/dev/null; then
      echo "Skip Webman reload: cached PID $WEBMAN_PID is not running"
      RESTART_WEBMAN=0
    fi
